...
export class SyncManager {
    ...
    async bootstrap() {
        ...
        await this.db.transaction('rw',
            [this.db.meta, this.db.areas, this.db.assets, this.db.trees, this.db.catalog_types, this.db.custom_fields],
            async () => {
                await Promise.all([
    ...
    async enqueueAssetCreate({ areaId, objectTypeId, censusCode, notes, geometry, gpsAccuracy }) {
        ...
        const command = {
            ...
            geom: {
                ...geometry,
                properties: { survey_method: 'gps', gps_accuracy_m: gpsAccuracy ?? null },
            },
    ...
    async pushQueue() {
        ...
        try {
            ...
            await this.db.sync_queue.where('idempotency_key')
                ...
                .modify((c) => { c.status = 'PENDING'; c.attempts += 1; c.last_error = String(err.response?.status ?? err.message); });
            throw err;
        }
    ...
    async pullChanges() {
        ...
        while (hasMore) {
            ...
            const { data } = await axios.get('/api/v1/sync/changes', { params: { cursor } });
